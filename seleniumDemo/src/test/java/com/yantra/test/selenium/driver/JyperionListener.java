...
import com.yantra.test.selenium.Base;
/**
 * JyperionListener
 * 
...
public class JyperionListener implements ITestListener 
{
	...
	public void onTestSuccess(ITestResult result) 
	{
		...
		if (throwable != null) 
		{
			...
			this.nbExceptions++;
			Paragraph excep = new Paragraph(new Chunk(throwable.toString(),new Font(Font.TIMES_ROMAN, Font.DEFAULTSIZE, Font.UNDERLINE)).setLocalGoto("" + throwable.hashCode()));
	...
	public void onTestFailure(ITestResult result) 
	{
		...
		String fileName="screenshot"+(new Random().nextInt())+".png";
		String file="test-output"+File.separator+fileName;
		try 
		{
			BaseClass.takeSnapShot(Base.driver, file);
		} 
		catch (Exception e) 
		{
			e.printStackTrace();
		...
		if (throwable != null) 
		{
		    ...
		    imdb.setAction(new PdfAction("file://"+fileName));
	...
	public void onStart(ITestContext context) 
	{
		...
		try 
		{
			File file=new File("test-output"+File.separator+context.getName()+".pdf");
		...
		this.document.open();
		Paragraph p = new Paragraph(context.getName() + " TESTNG RESULTS",FontFactory.getFont(FontFactory.HELVETICA, 20, Font.BOLD, new Color(0, 0, 255)));
		try 
		{
			this.document.add(p);
	...
	public void onFinish(ITestContext context) 
	{
		...
		Paragraph p = new Paragraph("EXCEPTIONS SUMMARY",FontFactory.getFont(FontFactory.HELVETICA, 16, Font.BOLD, new Color(255, 0, 0)));
		try 
		{
			this.document.add(p);
		} 
		...
		for(Integer key : keys) 
		{
			...
			Chunk chunk = new Chunk(throwable.toString(),FontFactory.getFont(FontFactory.HELVETICA, 12, Font.BOLD, new Color(255, 0, 0)));
